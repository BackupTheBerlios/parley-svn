...
sub authenticate : Path('/user/authenticate') {
    ...
    my $regauth = $c->model('ForumDB')->table('registration_authentication')->search(
        {
    ...
    $c->stash->{authed_user} = $c->model('ForumDB')->table('person')->search(
        {
...
sub _username_exists {
    ...
    my $user = $c->model('ForumDB')->table('authentication')->search(
        username => $username,
    );
...
sub _forumname_exists {
    ...
    my $user = $c->model('ForumDB')->table('person')->search(
...
sub _new_user {
    ...
    if (not scalar(@messages)) {
        ...
        eval {
            # start a transaction
            $c->model('ForumDB')->table('authentication')->storage->txn_begin;

            # add authentication
            $new_auth = $c->model('ForumDB')->table('authentication')->create(
                {
            ...
            # add person
            $new_person = $c->model('ForumDB')->table('person')->create(
                {
            ...
            # commit everything
            $c->model('ForumDB')->table('authentication')->storage->txn_commit;
        };
        # any errors?
        if ($@) {
            ...
            # rollback
            eval { $c->model('ForumDB')->table('authentication')->storage->txn_rollback };
        }
...
sub _send_auth_email {
	...
	$invitation = $c->model('ForumDB')->table('registration_authentication')->create(
        {
