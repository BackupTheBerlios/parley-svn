...
sub view : Local {
    ...
    $c->stash->{post_list} = $c->model('ForumDB')->table('post')->search(
        {
...
sub add : Local {
    ...
    if (not Forum::App::Helper->is_authenticted($c)) {
...
sub _login_if_required {
    ...
    if( not Forum::App::Helper->is_logged_in($c) ) {
...
sub _add_new_topic {
    ...
    if ($results || !$DFV) {
        # create a new thread
        my $new_thread = $c->model('ForumDB')->table('thread')->create(
            {
        ...
        my $new_post = $c->model('ForumDB')->table('post')->create(
            {
