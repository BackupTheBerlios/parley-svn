...
#
__PACKAGE__->config( YAML::LoadFile(__PACKAGE__->config->{'home'}.'/forum.yml') );
...
#            $c->log->dumper( $c->session->{authed_user} );
        ...
        # XXX for now do it the long way ...
            ...
            my $auth = $c->model('ForumDB')->table('authentication')->search(
                'username' => $c->user->user->username(),
            )->first();
            # get the corresponding persion record
            my $person = $c->model('ForumDB')->table('person')->search(
    ...
    if (defined $c->req->param('post')) {
        ...
        $c->stash->{current_post} = $c->model('ForumDB')->table('post')->search(
    ...
    if (defined $c->req->param('thread')) {
        $c->log->debug('setting: current_thread');
        $c->stash->{current_thread} = $c->model('ForumDB')->table('thread')->search(
    ...
    if (defined $c->req->param('forum')) {
        ...
        $c->stash->{current_forum} = $c->model('ForumDB')->table('forum')->search(
