package Phorum::Controller::User;
...
sub authenticate : Path('/user/authenticate') {
    ...
    my $regauth = $c->model('PhorumDB')->table('registration_authentication')->search(
        {
    ...
    $c->stash->{authed_user} = $c->model('PhorumDB')->table('person')->search(
        {
...
sub _username_exists {
    ...
    my $user = $c->model('PhorumDB')->table('authentication')->search(
        username => $username,
    );
...
sub _forumname_exists {
    ...
    my $user = $c->model('PhorumDB')->table('person')->search(
...
sub _new_user {
    ...
    if (not scalar(@messages)) {
        ...
        eval {
            # start a transaction
            $c->model('PhorumDB')->table('authentication')->storage->txn_begin;

            # add authentication
            $new_auth = $c->model('PhorumDB')->table('authentication')->create(
                {
            ...
            # add person
            $new_person = $c->model('PhorumDB')->table('person')->create(
                {
            ...
            # commit everything
            $c->model('PhorumDB')->table('authentication')->storage->txn_commit;
        };
        # any errors?
        if ($@) {
            ...
            # rollback
            eval { $c->model('PhorumDB')->table('authentication')->storage->txn_rollback };
        }
...
sub _send_auth_email {
	...
	$invitation = $c->model('PhorumDB')->table('registration_authentication')->create(
        {
    ...
    $c->email(
        header => [
            From    => q{Phorum Registration <phorum@herlpacker.co.uk>},
            To      => $person->email(),
...
Phorum::Controller::User - Catalyst Controller
